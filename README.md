...
片庫上方那個輸入框就是搜尋 —— **打字即時篩片庫，按 Enter 才上網找**，
結果從上方展開成懸浮面板。設定裡可以指定**偏好來源**（例如 YouTube），
之後搜尋就會把它排最前面；那個來源沒結果時其餘照常遞補。

整部作品的每一集要一次加進來：用搜尋面板最下面的
**「貼上 YouTube 播放清單網址整份匯入」**——一份清單就是一整季，
比一集一集搜快得多，也幾乎不花額度。

## 用外部 App 開 YouTube

設定裡填一個 URL 樣板（例如 `tubebrowser://open?url={url}`），
YouTube 連結就會交給那個 App 開 —— **網頁做不到 YouTube 背景播，那些原生 App 可以**。
可用的替換符：`{url}`（編碼過）、`{rawurl}`、`{nohttps}`。
卡片的 ⋯ 也會多一個「用外部 App 開」可以臨時切換。

## 分集歸到作品底下

一部作品的各集不會散成一堆卡片。網格上只出現「作品」，卡片會標「N 集」，
點下去才看到分集清單，看過的集數會壓暗。

- **補齊集數**（卡片 ⋯ 裡）：貼一次 YouTube 播放清單網址，整季一次加進來，
  全部掛在那部底下。重複的集數自動略過，所以之後出新集再貼一次也不會重複
- **合併**（選取模式）：同一部散成好幾筆時，勾起來按「合併」，挑一筆當作品，
  其餘變成它的分集。**不會刪任何資料**，只是改「所屬作品」欄，清掉就還原

## 卡片與待追

卡片就是一張封面 —— 標題與進度疊在圖片上，**點封面就是開播**，
編輯／查中文名／去哪裡看／刪除收在右上的 ⋯ 裡。進度**點數字直接輸入**。
追連載時落後幾集會在封面上標一顆小標籤。

想一次刪很多部：片庫右上角的「選取」，點封面勾選，或按「全選」。


- **小說與漫畫的「去哪裡看」**（⋯ 選單裡）：Bangumi 給的是資料頁不是內文，
  這裡把書名帶到起點、なろう、Kobo、博客來等平台的搜尋頁；找到之後把網址存回來，
  下次點卡片就直接到那裡。**這個站不存也不抓任何內文，它是連結目錄**
- **「待追」分頁**列出已經播了但你還沒追上的作品，卡片上會寫「落後 N 集」。
  沒有落後的作品時這個分頁不會出現
- **查中文名**（⋯ 選單裡）：日文或英文標題認不出來時，拿它去 Bangumi 與 TMDB
  查人家登記好的中文名。**這不是翻譯** —— 查不到就是查不到，不會編一個給你

## 整理片庫

設定裡有兩個維護工具：

- **找出重複**：連結相同或名稱相同就算同一部，每組留最早加入的那筆
- **匯出 CSV**：丟進 Excel／Google 試算表自己排序統計。
  跟 JSON 備份的分工是「CSV 是看的、JSON 是還原用的」，兩個都留著
...
| **手機分享** | 在任何 app 點分享 → 選「墨影」 | Android（iOS 不支援分享目標） |
...
關掉螢幕或切到別的 app 時聲音繼續播，設定裡可以關閉。

**只對影片直鏈有效**（`.mp4`／`.m3u8` 那種）。YouTube、BiliBili 這類內嵌播放器
是對方的頁面，我們碰不到它的播放器 —— 這是網頁的硬邊界，不是還沒做。

> Brave、Tube Browser 那些能讓 YouTube 在背景播，是因為它們是 **App Store 上的
> 原生 App**，可以向 iOS 申請背景音訊權限。網頁拿不到那個權限。

作法是進背景時把聲音交給一個隱藏的 `<audio>` 接手 —— iOS 不讓 `<video>`
在背景播，但 `<audio>` 可以。回到前景會從剛才聽到的位置接回影片。
...
- **TMDB**（繁中片名、海報、集數）—— 需要免費金鑰，存在自己的瀏覽器裡
