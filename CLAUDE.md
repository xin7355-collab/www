...
### 分集歸到作品底下（第 21 欄 `parent`）

- **`parent` 存的是父作品的「名稱」**，不是列號也不是 ID。列號會因為刪除
  整批位移；而名稱在試算表裡看得懂 —— 這份資料使用者自己也會打開來看。
  代價是改名要順手更新子項，`page.tsx` 的 rename 流程有做這件事，
  少了它那些集數會斷鏈變成一堆獨立卡片
- 網格只顯示「作品」（`parent` 為空），分集藏在裡面 ——
  否則匯入一份 279 集的播放清單，片庫就變成 279 張卡片
- **找不到父作品的分集當成獨立作品顯示**：父項被刪掉時，那些集數不該
  就此從畫面上消失、只剩試算表裡看得到
- 有分集的卡片，**點下去是看有哪幾集而不是直接播某一集**

### 補齊集數只走播放清單，不逐集搜尋

搜尋一次要 100 單位額度，一部 279 集的作品逐集搜等於 27,900 單位 ——
一天只有 10,000。播放清單一頁只要 1 單位，整季通常不到 10 單位。
補第二次時用 `watchUrl` 比對略過已經有的，只補新出的那幾集。

### 評分拿掉了，但 Sheet 的 H 欄留著

刪掉那一欄會讓後面 13 欄整批往前位移，既有資料全部對不上。
UI（表單欄位、排序選項）移除，欄位本身當作廢欄保留。
...
`globals.css` 的 `.field` 在觸控裝置上用 `max(16px, 0.875rem)`。
**iOS Safari 只要輸入框字級小於 16px，一聚焦就會自動放大整頁** ——
畫面會突然被推歪、標題被切掉，看起來像版面壞了，但其實沒有任何溢出。
用 `max()` 而不是寫死 16px，是為了仍然尊重使用者調大的字級設定。

### 藏起來的入口等於沒有這個功能

「用外部 App 開」原本只在設定過樣板時才出現在 ⋯ 選單裡 —— 結果使用者
根本不知道有這回事，回報「我沒看到這個功能」。現在一律顯示，
沒設定就把人帶去設定頁。播放器底部也放了一顆，因為正在看內嵌播放器的
當下最想切出去。

### 搜尋沒有按鈕，就是頁首那個輸入框

搜尋框放在頁首、標題旁邊那塊本來就空著的地方 —— 它是最常用的東西，
不該再往下佔一整行。那個 flex 子元素**必須加 `min-w-0`**，否則不會縮到
內容以下，會把頁首撐爆。
...
- **那個 `<audio>` 必須先在使用者手勢裡「解鎖」**（播一下立刻暫停）。
  iOS 不准程式去播一個從來沒在手勢裡播過的媒體元素，而交接發生在
  `visibilitychange` 裡 —— 那不是手勢。少了這步，iOS 上背景會整個沒聲音
  （第一版就是這樣壞的，Chromium 測不出來，它沒有這條限制）
